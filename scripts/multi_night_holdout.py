#!/usr/bin/env python3
"""Wave 2 step: train Head A on ≥2 Sleep-EDF nights, hold out other subject(s).

Protocol:
  Train nights: SC4001 + SC4011 (distinct cassette subjects; equal per-night balance)
  Primary holdout: SC4002 (same subject as wave-1 single-night holdout → comparable)
...
def main() -> None:
    ...
    metrics: Dict[str, Any] = {
        "train_balanced": eval_pred(y_tr, pred_tr, "train_balanced_multi_night"),
    ...
    run_dir = ROOT / "exports" / "head_a_multi_night"
    run_dir.mkdir(parents=True, exist_ok=True)
    head_path = run_dir / "head_a_binary_multi_night.pt"
    torch.save(head.state_dict(), head_path)

    # Compare to prior single-night SC4001→SC4002 if present
    prior = {}
    ...
    if tighten_path.exists():
        ...
        # best-effort pull of stride metrics
    ...
    run_manifest = {
        "task": "binary_drowsy_vs_hypnagogic_multi_night_holdout",
        ...
        "protocol_note": (
            "Per-night undersample then equalize to min night per-class so SC4011 "
            "N1 abundance does not dominate; subject holdout (not same-subject night2)."
        ),
        "slice_recipe": {
    ...
    docs = ROOT / "docs" / "multi_night_holdout.md"
    ...
    docs.write_text(
        f"""# Multi-night holdout — Head A binary (drowsy vs hypnagogic)
...
Multi-night SC4002 all-overlap macro-F1 = {primary['all_labeled_overlap']['macro_f1']:.4f}; stride×4 = {sc4002_s4['macro_f1']:.4f}.
...
"""
    ...
    summary = {
        ...
        "artifacts": {
            "script": "scripts/multi_night_holdout.py",
            "docs": "docs/multi_night_holdout.md",
            "metrics": "exports/head_a_multi_night/metrics_summary.json",
            "run_manifest": "exports/head_a_multi_night/run_manifest.json",
            "head": "exports/head_a_multi_night/head_a_binary_multi_night.pt",
        },
        "created_utc": run_manifest["created_utc"],
    }
