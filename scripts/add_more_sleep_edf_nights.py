#!/usr/bin/env python3
"""Wave2 step more_sleep_edf_nights: add SC4011 + SC4031 windows + provenance."""
...
def main() -> None:
    ...
    results: Dict[str, Any] = {"nights": [], "created_utc": datetime.now(timezone.utc).isoformat()}
    ...
    # provenance for raw EDFs added to cache
    provenance = {
        "step": "more_sleep_edf_nights",
    ...
    prov_path = cassette.parent / "provenance_more_nights.json"
    prov_path.write_text(json.dumps(provenance, indent=2) + "\n")
    results["provenance_path"] = str(prov_path)

    # refresh package rollup manifest
    nights_all = ["SC4001", "SC4002", "SC4011", "SC4031"]
    rollup: Dict[str, Any] = {
    ...
    results["rollup"] = {k: rollup[k] for k in ("nights", "updated_utc") if k in rollup}
    results["per_night"] = {k: rollup[k] for k in ("sc4001", "sc4002", "sc4011", "sc4031") if k in rollup}

    out_summary = ROOT / "exports/more_sleep_edf_nights_summary.json"
