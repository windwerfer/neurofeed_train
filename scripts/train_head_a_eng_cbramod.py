#!/usr/bin/env python3
"""Train frozen A-eng head on expanded engagement corpus (CBraMod).
...
OUT = ROOT / "exports/head_a_eng_train_cbramod"
EMB_CACHE = OUT / "emb_cache"
DOCS = ROOT / "docs/head_a_eng_train_cbramod.md"
...
def ensure_emb_cache(
    ...
    meta: Dict[str, Any] = {"packs": [], "encoder": "CBraMod", "frozen": True}
...
def write_docs(metrics: Dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        f"**Step:** `head_a_eng_train_cbramod`  ",
    ...
    lines += [
        ...
        f"- Metrics: `exports/head_a_eng_train_cbramod/metrics_summary.json`",
...
def write_dual_stub(cbramod_metrics: Dict[str, Any]) -> None:
    """Write / update dual-encoder doc with CBraMod filled; REVE pending."""
    test = cbramod_metrics["test"]
    lines = [
        "# Head A-eng dual-encoder fit vs misfit (CBraMod + REVE)",
        "",
        f"**Updated (UTC):** {cbramod_metrics['created_utc']}",
        "",
        "## Task",
        "",
        ...
        "",
        "| Encoder | test n | test macro-F1 | Δ vs chance | ship_candidate |",
        "|---------|-------:|--------------:|------------:|:--------------:|",
        f"| **CBraMod** | {test['n']} | {test['macro_f1']:.3f} | {test['macro_f1']-CHANCE_F1:+.3f} | **{cbramod_metrics['ship_candidate']}** |",
        "| **REVE** | — | pending Kaggle T4 | — | pending |",
        "",
        "## CBraMod detail",
        "",
        f"- Docs: `docs/head_a_eng_train_cbramod.md`",
        f"- Exports: `exports/head_a_eng_train_cbramod/`",
        f"- ship_reason: {cbramod_metrics['ship_reason']}",
        "",
        "## REVE",
        "",
        ...
        "",
        cbramod_metrics["recommendation"],
        "",
        "_REVE row will be filled when Kaggle run completes._",
        "",
    ]
...
def main() -> None:
    ...
    if not args.skip_encode:
        wpath = find_weights()
        print(f"loading CBraMod from {wpath}", flush=True)
        encoder = FrozenCBraModEncoder(weights_path=wpath, source_sr=TARGET_SR).to(device)
        encoder.eval()
    ...
    metrics = {
        ...
        "weights_sha256": sha256(find_weights()),
    }
    ...
    new = f'ENG_TRAIN_STATUS = "trained_cbramod_ship_{str(ship).lower()}"'
    if old in txt:
